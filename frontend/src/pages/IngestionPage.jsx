...
import { listSources, deleteSource, uploadStaticFile, listSilverTables, runQuery, getBlueprints, setBlueprints, getDQRules, addDQRule } from '../api/client.js';
...
function PipelinesTab() {
  ...
  const load = () => { setLoading(true); listSources().then(setSources).catch(() => toast('Failed', 'error')).finally(() => setLoading(false)); };
  useEffect(() => { load(); }, []);
...
function QueryTab() {
  ...
  useEffect(() => { loadT(); }, []);
  const exec = async () => {
    if (!sql.trim()) return; setLoading(true); setError(null); setResult(null);
    ...
    <div style={{ display: 'flex', gap: 20, height: '100%', overflow: 'hidden' }}>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 16, flex: 2, minWidth: 320 }}>
        <div className="card" style={{ padding: 20, flexShrink: 0 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
            <h2>SQL Query</h2>
            <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>{time && <span style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>{time}s</span>}<button className="btn btn-primary" onClick={exec} disabled={loading}>{loading ? <span className="spinner" /> : '▶ Run'}</button></div>
          </div>
        ...
        <div className="card" style={{ padding: 20, flex: 1, overflow: 'auto' }}>
          ...
          {!loading && !error && result && result.length > 0 && <div style={{ overflow: 'auto', borderRadius: 10, border: '1px solid var(--border-subtle)' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.8rem' }}>
              <thead><tr style={{ background: 'var(--bg-elevated)' }}>{Object.keys(result[0]).map(c => <th key={c} style={{ padding: '10px 14px', textAlign: 'left', fontWeight: 600, color: 'var(--text-muted)', fontSize: '0.72rem', textTransform: 'uppercase', borderBottom: '1px solid var(--border-default)', whiteSpace: 'nowrap' }}>{c}</th>)}</tr></thead>
              <tbody>{result.map((row, i) => <tr key={i} style={{ borderBottom: '1px solid var(--border-subtle)', background: i % 2 ? 'hsla(222,28%,9%,0.4)' : 'transparent' }}>{Object.keys(result[0]).map(c => <td key={c} style={{ padding: '9px 14px', color: 'var(--text-secondary)', maxWidth: 240, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{row[c] === null ? <span style={{ color: 'var(--text-muted)', fontStyle: 'italic' }}>null</span> : String(row[c])}</td>)}</tr>)}</tbody>
            </table>
          </div>}
