# Relationship Mapping Node Documentation

The Relationship Mapping Node is an AI-powered component designed to discover and materialize relationships between entities in the knowledge graph. It uses Large Language Models (Gemini/Mistral) to evaluate the likelihood of a specific relationship type between a set of source entities and target entities.

## Implementation Details

### Backend Logic (`relationship_mapping_node.py`)

The backend is responsible for:
- **Parallel Evaluation**: Using a `ThreadPoolExecutor` to process multiple entities simultaneously, optimized for AI API throughput.
- **Interruptible Requests**: A custom polling mechanism ensures AI calls can be cancelled mid-flight if the user stops the execution.
- **Smart Sampling**: To stay within LLM context windows, it samples target entities while prioritizing "string targets" (user-typed concepts).
- **Neo4j Write-back**: Materializes approved relationships (based on a configurable score threshold) into the graph with `score`, `reason`, and `ai_generated: true` attributes.
- **Local Cache**: Uses a configuration-aware hash to cache evaluations, preventing redundant AI calls for the same entity sets.

#### [Backend Source](file:///d:/idp/HVE-OS/src/Logic/nodes/ai/relationship_mapping_node.py)
```python
"""
...
### Frontend UI (`RelationshipMappingNode.jsx`)

The frontend provides:
- **Dynamic Schema Discovery**: Automatically lists available attributes from upstream nodes for mapping.
- **Interactive Configuration**:
  - API Key management and connection testing.
  - Model selection (Mistral/Gemini).
  - Relation Type specification.
  - Multi-select attribute lists for source and target entity payloads.
- **Inter-Relationship Mode**: Toggle to discover relationships among entities in the same input stream.
- **Connection Status**: Visual indicators for source and target wire connectivity.

#### [Frontend Source](file:///d:/idp/HVE-OS/processing_layer_frntnd/frontend/src/components/nodes/ai/RelationshipMappingNode.jsx)
```javascript
...
|-----------|-------------|---------|
| `relationType` | The Cypher relationship type to be created (e.g., OWNS). | `RELATES_TO` |
| `minScore` | Confidence threshold (0.0 to 1.0) for creating a link. | `0.7` |
| `parallelWorkers` | Number of concurrent AI evaluation threads. | `5` |
| `aiProvider` | Choice between `gemini` and `mistral`. | `gemini` |
| `sourceFields` | Attributes from the source entity to include in the AI prompt. | `[]` |
| `targetFields` | Attributes from target entities to include in the AI prompt. | `[]` |

## Data Flow Diagram

```mermaid
graph LR
    SourceData["Source Entities (Handle)"] --> Node["Relationship Mapping Node"]
    TargetData["Target Entities (Handle)"] --> Node
    Node --> AI["LLM Evaluation (Score & Reason)"]
    AI --> Neo4j["Neo4j Knowledge Graph (MERGE)"]
    Node --> Output["Source Entities (Pass-through)"]
```
